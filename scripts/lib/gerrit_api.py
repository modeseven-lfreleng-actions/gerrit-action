...
def _parse_response(response: requests.Response, allow_non_json: bool = False) -> Any:
    ...
    Args:
        ...
        allow_non_json: If True, don't raise an error for non-JSON responses
    """
...
class GerritDevClient:
    ...
    def __init__(
        ...
        # Extract root URL (without API path prefix like /r or /gerrit)
        # This is needed because login endpoint is always at the root
        self._root_url = self._extract_root_url(self.base_url)
    ...
    def _extract_root_url(self, url: str) -> str:
        """
        Extract the root URL from a base URL that may include an API path prefix.

        For example:
            http://localhost:8080/r -> http://localhost:8080
            http://localhost:8080/gerrit -> http://localhost:8080
            http://localhost:8080 -> http://localhost:8080

        This is needed because the login endpoint in DEVELOPMENT_BECOME_ANY_ACCOUNT
        mode is always at the root (e.g., /login/?account_id=X), not under the
        API path prefix.
        """
        from urllib.parse import urlparse, urlunparse

        parsed = urlparse(url)
        # Return just scheme + netloc (host:port)
    ...
    def become_account(self, account_id: int) -> bool:
        ...
        """
        # Login endpoint is always at the root URL, not under API path prefix
        login_url = f"{self._root_url}/login/?account_id={account_id}"
    ...
    def become_admin(self) -> int:
        """
        Become an admin account, trying common admin account IDs.
        ...
        raise GerritAuthError("Failed to authenticate as any admin account")
